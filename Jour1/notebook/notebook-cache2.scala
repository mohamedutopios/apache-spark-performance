...
val df2018 = df.filter($"p_transacted_year" === 2018).repartition(30)
df2018.groupBy($"city_id").agg(sum($"amount").alias("total_sales")).show()

val df2017 = df.filter($"p_transacted_year" === 2017).repartition(30)
...
// MAGIC %scala
// Activation du cache
val dfCached = df.repartition(30).cache()

// Mesure du temps avec cache
val startTimeWithCache = nanoTime()

val df2018Cached = dfCached.filter($"p_transacted_year" === 2018)
df2018Cached.groupBy($"city_id").agg(sum($"amount").alias("total_sales")).show()

val df2017Cached = dfCached.filter($"p_transacted_year" === 2017)
